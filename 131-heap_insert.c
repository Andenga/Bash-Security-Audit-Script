...
heap_t *heap_insert(heap_t **root, int value)
{
	...
	if (*root == NULL)
	{
		*root = binary_tree_node(NULL, value); // Create a new node as the root
		return (*root);
	}
	...
	if (binary_tree_is_perfect(*root) || !binary_tree_is_perfect((*root)->left))
	{
		if ((*root)->left)
		{
			new_node = heap_insert(&((*root)->left), value); // Insert into the left subtree
			swap(root, &((*root)->left)); // Swap nodes if necessary to maintain the heap property
			return (new_node);
		}
		else
		{
			new_node = (*root)->left = binary_tree_node(*root, value); // Create a new node as the left child
			swap(root, &((*root)->left)); // Swap nodes if necessary
			return (new_node);
		}
	}
	...
	if ((*root)->right)
	{
		new_node = heap_insert(&((*root)->right), value); // Insert into the right subtree
		swap(root, (&(*root)->right)); // Swap nodes if necessary
		return (new_node);
	}
	else
	{
		new_node = (*root)->right = binary_tree_node(*root, value); // Create a new node as the right child
		swap(root, &((*root)->right)); // Swap nodes if necessary
		return (new_node);
	}
